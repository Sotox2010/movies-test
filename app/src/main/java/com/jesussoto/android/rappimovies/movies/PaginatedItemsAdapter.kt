...
internal class PaginatedItemsAdapter(
        private var items: MutableList<DisplayableItem>?): RecyclerView.Adapter<PaginatedItemsAdapter.ViewHolder>() {
    ...
    fun replaceData(items: MutableList<DisplayableItem>?) {
        this.items = items
        notifyDataSetChanged()
    }

    fun appendData(items: List<DisplayableItem>) {
        this.items!!.addAll(items.asIterable())
        notifyDataSetChanged()
    }
